...
public class BaseControllerTestIT {
    ...
    String str="{\"client_id\": \""+clientConfig.getClientId()+ "\",\"client_secret\": \""+ clientConfig.getClientSecret()+"\",\"grant_type\":\"client_credentials\", \"scope\":\""+scope+"\"}";
        MvcResult result = this.mockMvc.perform(post(tokenBasePath+"/open-banking/v3/token")
                .contentType(MediaType.APPLICATION_JSON)
                .content(str)
                .param("scope",scope)
                .param("client_id", clientConfig.getClientId())
                .param("client_secret", clientConfig.getClientSecret())
